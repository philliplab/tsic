...
format_lb_med_ub <- function(lb_med_ub){
  ...
  for (cname in names(lb_med_ub)){
    if (is.null(lb_med_ub[[cname]])){
      ...
      val <- ifelse(is.numeric(lb_med_ub[[cname]]), round(lb_med_ub[[cname]], 2), lb_med_ub[[cname]])
    }
    outp <- rbind(outp,
...
The probability that the infection date was between `r as.Date(17987, origin = '1970-01-01')` and `r as.Date(18017, origin = '1970-01-01')` is `r lb_med_ub$aoc_left_of_date[2] - lb_med_ub$aoc_left_of_date[1]`
